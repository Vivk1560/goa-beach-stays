...
/** lucide-react dropped brand/logo icons — inline replacements, same stroke style */
...
export function Footer() {
  ...
  // Developer credit links to the developer's own WhatsApp (not the business number),
  // same convention as mahabaleshwarvillastays.com.
  const developerWhatsappHref = whatsappUrl(
    "Hi Vivaan, I found your work on Goa Beach Stays and wanted to get in touch.",
    siteConfig.developerWhatsapp,
  )
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            <li>
              ...
              >
                <MessageCircle className="size-4" /> {siteConfig.contact.whatsappNumber}
              </a>
            </li>
            <li>
              <a href={callUrl()} className="flex items-center gap-2 hover:text-terracotta">
                <Phone className="size-4" /> {siteConfig.contact.callNumber}
              </a>
            </li>
            <li>
              <a href={`mailto:${siteConfig.contact.email}`} className="flex items-center gap-2 hover:text-terracotta">
          ...
          <div className="mt-4 flex gap-3">
            <a
              ...
              className="rounded-full bg-white/10 p-2 hover:bg-terracotta"
            >
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-white/80">
            {footerNav.quickLinks.map((l) => (
              <li key={l.href}>
                <Link href={l.href} className="hover:text-terracotta">
                  {l.label}
                </Link>
              </li>
            ))}
        ...
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 text-xs text-white/70 sm:flex-row">
          <p>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="font-medium text-white underline-offset-2 hover:text-terracotta hover:underline"
            >
          ...
          <div className="flex gap-4">
            ...
            <Link href="/terms-of-service" className="hover:text-terracotta">
              Terms of Service
            </Link>
            <Link href="/sitemap.xml" className="hover:text-terracotta">
              Sitemap
            </Link>
          </div>
        </div>
